<script setup>
  ...
  const { t } = useI18n(); //Implementa la internacionalización
  const route = useRoute(); //Obtiene la ruta actual
  const router = useRouter(); //Controlador de rutas
  const store = usePublishingStore(); //Se comunica con la capa de aplicación
